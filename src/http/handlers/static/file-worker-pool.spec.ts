...
describe('FileWorkerPool', () => {
  ...
  describe('Error Handling', () => {
    it('should complete in-flight task before termination', async () => {
      pool = new FileWorkerPool(1);

      // Start a task and await it before terminating
      const result = await pool.readFile(testFilePath);

      // Verify task completed successfully
      expect(result).toBeDefined();
      expect(result.byteLength).toBeGreaterThan(0);

      // Now terminate the pool
      await pool.terminate();
    });

    it('should terminate idle pool cleanly', async () => {
      pool = new FileWorkerPool(1);

      // Terminate without any pending tasks
      await pool.terminate();

      // Verify subsequent tasks are rejected
      await expect(pool.readFile(testFilePath)).rejects.toThrow();
    });
