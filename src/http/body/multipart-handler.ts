...
export interface MultipartField {
  ...
  file?: {
     ...
     *
     * Note: Check stream.truncated after consuming the stream to determine
     * if the file was truncated due to size limits. The truncated property
     * is only set by Busboy after the stream ends.
     */
    stream: Readable;
  };
}
...
export class MultipartFormHandler {
  ...
  private async executeHandler(handler: MultipartHandler, field: MultipartField): Promise<void> {
    ...
    // Set multipartPromise BEFORE resuming to prevent race condition
